...
class Channel_Embeddings(nn.Module):
    ...
    """
    def __init__(self, patchsize, img_size, in_channels, embeddings_dropout_rate=0.1, **kwargs):
        super().__init__()
...
class Attention_org(nn.Module):
    def __init__(self, vis, channel_num, KV_size=960, num_heads=4, attention_dropout_rate=0.1,  **kwargs):
...
class Mlp(nn.Module):
    def __init__(self, in_channel, mlp_channel, dropout_rate=0,  **kwargs):
...
class Block_ViT(nn.Module):
    def __init__(self, vis, channel_num, expand_ratio=4, KV_size=960, **kwargs):
        ...
        self.channel_attn = Attention_org(vis, channel_num, KV_size=KV_size, **kwargs)
        ...
        self.ffn1 = Mlp(channel_num[0], channel_num[0]*expand_ratio, **kwargs)
        self.ffn2 = Mlp(channel_num[1], channel_num[1]*expand_ratio, **kwargs)
        self.ffn3 = Mlp(channel_num[2], channel_num[2]*expand_ratio, **kwargs)
        self.ffn4 = Mlp(channel_num[3], channel_num[3]*expand_ratio, **kwargs)
...
class Encoder(nn.Module):
    def __init__(self, vis, channel_num, num_layers=4, **kwargs):
        ...
        for _ in range(num_layers):
            layer = Block_ViT(vis, channel_num, **kwargs)
...
class ChannelTransformer(nn.Module):
    def __init__(self, vis, img_size, channel_num=[64, 128, 256, 512], patchSize=[32, 16, 8, 4], **kwargs):
        super().__init__()
        ...
        self.embeddings_1 = Channel_Embeddings(self.patchSize_1, img_size=img_size,    in_channels=channel_num[0], **kwargs)
        self.embeddings_2 = Channel_Embeddings(self.patchSize_2, img_size=img_size//2, in_channels=channel_num[1], **kwargs)
        self.embeddings_3 = Channel_Embeddings(self.patchSize_3, img_size=img_size//4, in_channels=channel_num[2], **kwargs)
        self.embeddings_4 = Channel_Embeddings(self.patchSize_4, img_size=img_size//8, in_channels=channel_num[3], **kwargs)
        self.encoder = Encoder(vis, channel_num, **kwargs)
    ...
    def forward(self,x):
        ...
        # print("emb1 size:", emb1.size())
        # print("emb2 size:", emb2.size())
        # print("emb3 size:", emb3.size())
        encoded1, encoded2, encoded3, encoded4, attn_weights = self.encoder(emb1,emb2,emb3,emb4)  # (B, n_patch, hidden)
        # print("encoded1 size:", encoded1.size())
        # print("encoded2 size:", encoded2.size())
        # print("encoded3 size:", encoded3.size())
